...
const AdminParts = () => {
  ...
  const purchasedPartsData = filteredParts.filter(part => part.stock_quantity > 0 || purchases?.some(p => p.part_id === part.id)).map(part => {
    ...
    const {
      ...
      totalResults: catalogTotalResults
    } = usePagination(filteredParts, 10);

    // Pagination for Purchased Parts
    const {
      currentPage: purchasedPage,
      totalPages: purchasedTotalPages,
      currentData: purchasedData,
      onPageChange: onPurchasedPageChange,
      totalResults: purchasedTotalResults
    } = usePagination(purchasedPartsData, 10);
