...
export default function ServicesPage() {
  return (
    <div style={{ background: '#FFFFFF', minHeight: '100vh', paddingTop: '40px' }}>
      ...
      <section className="serv-hero">
        <div className="container-pro">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="badge-pro" style={{ margin: '0 auto 1.5rem' }}><Settings size={12} /> Institutional Services</div>
            <h1 style={{ fontSize: 'clamp(2.2rem, 6vw, 4rem)', fontWeight: 900, lineHeight: 1.1, marginBottom: '1.5rem' }}>
              Moin Motors <span style={{ color: 'var(--accent-red)' }}>Service Standards.</span>
            </h1>
            <p style={{ fontSize: '1.2rem', color: '#666', maxWidth: '650px', margin: '0 auto', lineHeight: 1.6 }}>
              Dewas mein car aur bike repair ka naya level. Hum quality, technology aur digital transparency par koi compromise nahi karte.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <div className="container-pro">
        <div className="serv-grid-box">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }} className="serv-mini-card">
              <div style={{ color: 'var(--accent-red)', marginBottom: '1rem', display: 'flex', justifyContent: 'center' }}>
                <item.icon size={32} />
              </div>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 900, marginBottom: '0.3rem' }}>{item.title}</h3>
              <p style={{ fontSize: '0.8rem', color: '#999', fontWeight: 700, textTransform: 'uppercase' }}>{item.val}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <section style={{ padding: '4rem 0' }}>
        <div className="container-pro">
          {detailedServices.map((s, i) => (
            <div key={i} className="serv-row" style={{ direction: s.reversed ? 'rtl' : 'ltr' }}>
              <motion.div 
                ...
                style={{ direction: 'ltr' }}
              >
                <div style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--accent-red)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.8rem' }}>
                  {s.subtitle}
                </div>
                <h2 style={{ fontSize: '2.5rem', fontWeight: 900, lineHeight: 1.1, marginBottom: '1.5rem' }}>{s.title}</h2>
                <p style={{ fontSize: '1.05rem', color: '#555', lineHeight: 1.7, marginBottom: '2rem' }}>{s.desc}</p>
                <div className="serv-points" style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', marginBottom: '2.5rem' }}>
                  {s.points.map((p, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: '0.95rem', fontWeight: 700, color: '#111' }}>
                       <CheckCircle2 size={18} color="var(--accent-red)" /> {p}
                    </div>
                  ))}
                </div>
                <Link to="/contact" className="btn-red" style={{ padding: '14px 36px', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 10 }}>
      ...
      <section style={{ padding: '6rem 0', background: '#111' }}>
        <div className="container-pro" style={{ textAlign: 'center' }}>
           <h2 style={{ color: 'white', fontSize: '3rem', fontWeight: 900, marginBottom: '1.5rem' }}>Moin Motors <br />Standard Apnao.</h2>
           <p style={{ color: 'rgba(255,255,255,0.5)', maxWidth: '500px', margin: '0 auto 2.5rem' }}>
             Ab Dewas mein car service ke liye darna nahi, digital billing aur transparency ka faida uthao.
           </p>
           <Link to="/contact" className="btn-red" style={{ padding: '16px 48px', fontSize: '1.1rem', textDecoration: 'none' }}>
